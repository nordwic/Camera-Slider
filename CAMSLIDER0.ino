...
#define SCREEN_WIDTH 128  // Высота и ширина дисплея
...
void oledDisplayCenter(const char* text, int x, int y) {
  ...
  display.print(text);  // Вывод текста
}
void Switch() {
...
void setup() {
  ...
  Home();  // Возваращает штатив в исходное положение
...
void loop() {
  ...
  if (flag == 4) {
    ...
    // Установка конкретной позиции камеры
